...
import { collection, query, where, or, limit, orderBy, onSnapshot, getDocs } from 'firebase/firestore';
...
export function SuperAdminDashboard({ userProfile }: { userProfile: DocumentData }) {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
        try {
            const queries = {
              ...
              assignments: collection(firestore, 'assignments'),
            };
            
            const [challengesSnap, studentsSnap, groupsSnap, staffSnap, assignmentsSnap] = await Promise.all([
                getDocs(queries.challenges),
                getDocs(queries.students),
                getDocs(queries.groups),
                getDocs(queries.staff),
                getDocs(queries.assignments),
            ]);
    ...
    const recentAssignmentsQuery = query(collection(firestore, 'assignments'), orderBy('assignedAt', 'desc'), limit(5));
    const unsubRecent = onSnapshot(recentAssignmentsQuery, (snapshot) => {
        if(isMounted) {
