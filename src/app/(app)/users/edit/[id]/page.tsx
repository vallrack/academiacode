...
import { doc, getDoc, updateDoc, collection, DocumentData, Query, serverTimestamp } from 'firebase/firestore';
...
export default function EditUserPage() {
  ...
  useEffect(() => {
    if (!firestore || !userIdStr) return;

    const fetchUser = async () => {
      setLoading(true);
      const userDocRef = doc(firestore, 'users', userIdStr);
      try {
    ...
    fetchUser();
  }, [firestore, userIdStr, toast, router]);
  ...
  const handleUpdate = async () => {
    if (!firestore || !userIdStr || !currentUser) {
        toast({ variant: "destructive", title: "Error de autenticación", description: "No estás autenticado para realizar esta acción." });
        return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/set-custom-claims', {
        ...
        body: JSON.stringify({ uid: userIdStr, role }),
      });
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-6">
            ...
            <div className="grid gap-3"><Label htmlFor="email">Correo Electrónico</Label><Input id="email" type="email" value={email} readOnly disabled /></div>
