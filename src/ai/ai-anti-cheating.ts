...
const AIAntiCheatingOutputSchema = z.object({
  report: z.string().describe('Un reporte detallado en español. La estructura del reporte DEBE ser: Primero, un bloque de código markdown con el código original del estudiante. Segundo, el análisis de la IA.'),
  riskAssessment: z
    .string()
...
const aiAntiCheatingPrompt = ai.definePrompt({
  ...
  ## Salida Final
  - Tu respuesta DEBE estar completamente en español. Proporciona un 'report' que siga ESTRICTAMENTE la siguiente estructura:
    1. Un bloque de código markdown que contenga el código original COMPLETO del estudiante. Usa tres comillas invertidas (\`\`\`) para abrir y cerrar el bloque.
    2. Después del bloque de código, proporciona tu análisis detallado de la solución, abordando la lógica, posibles trampas y eficiencia.
